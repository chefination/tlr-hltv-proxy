Here's the code for api/team.js — create the file and paste this:
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (resp.code === 200 && resp.body.length > 3000) {
      ...
      try {
        const mapResp = await fetchPage('https://www.hltv.org/stats/teams/maps/' + id + '/-');
        if (mapResp.code === 200) {
          result.maps = {};
          for (const map of ['Mirage','Inferno','Anubis','Nuke','Ancient','Dust2','Vertigo']) {
            const mm = mapResp.body.match(new RegExp(map + '[\\s\\S]*?(\\d+)%'));
            if (mm) result.maps[map] = parseInt(mm[1]);
          }
        }
      } catch(e) {}
